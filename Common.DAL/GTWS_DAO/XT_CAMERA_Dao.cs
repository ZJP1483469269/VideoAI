...
namespace TLKJ.DAO
{
    public class XT_CAMERA_Dao : BaseDao<XT_CAMERA>
    {
        ...
        public XT_CAMERA FindItem(string cKeyID)
        {
            XT_CAMERA vo = null;
            DataTable dtRows = DbManager.QueryData("SELECT * FROM xt_camera WHERE DEVICE_ID='" + cKeyID + "'");
            if ((dtRows != null) && (dtRows.Rows.Count > 0))
            {
                vo = new XT_CAMERA();
                ReadDB(vo, dtRows);
        ...
        {
            return DbManager.Query("*", "XT_CAMERA", cWhereParm, cOrderBy, iPageNo, iPageSize);
        }
    }
}
