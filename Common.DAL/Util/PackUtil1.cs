...
namespace TLKJ.DAO
{
    public class PackUtil
    {
        public static String Pack(String cTABLE_ID, DataTable dtRows, String cRootDir, String cSYSID)
        {
            ...
            DataTable dtFieldConfig = DbManager.GetDataTable("SELECT FIELD_NAME,ISKEY FROM S_ETL_FIELD WHERE TABLE_ID='" + cTABLE_ID + "'");
            for (int j = 0; (dtRows != null) && (j < dtRows.Rows.Count); j++)
            {
                ...
                DataTable dtFiles = DbManager.GetDataTable("SELECT ID,TEXT,URL FROM S_UPLOAD WHERE RES_ID='" + cKEY_ID + "'");

                for (int k = 0; (dtFiles != null) && (k < dtFiles.Rows.Count); k++)
                {
                    String cID = StringEx.getString(dtFieldConfig, k, "ID").ToUpper();
                    String cText = StringEx.getString(dtFieldConfig, k, "TEXT").ToUpper();
                    String cUrl = StringEx.getString(dtFieldConfig, k, "URL").ToUpper();
        ...
        public static int UpdateDB(List<ExportObject> Rows, String cRootDir)
        {
            ...
            for (int i = 0; i < Rows.Count; i++)
            {
                ...
                String cTableName = DbManager.GetValue("SELECT TABLE_NAME FROM S_ETL_TABLE WHERE TABLE_ID='" + cTABLE_ID + "'");
                DataTable dtFieldList = DbManager.GetDataTable("SELECT FIELD_NAME,ISKEY FROM S_ETL_FIELD WHERE TABLE_ID='" + cTABLE_ID + "'");
            ...
            return DbManager.ExeSql(sqls).Code;
        }
    }
}
