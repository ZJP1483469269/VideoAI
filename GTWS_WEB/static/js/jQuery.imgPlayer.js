...
;(function($) {
	...
	settings = jQuery.extend(this,{
		delay:		          5000, 			//停留时间-----------------图片----------
		fadeOutTime:          1200, 				//淡出时间(ms)
		fadeInTime:           1200, 	    		//淡入时间(ms)
		height:               118,  			//默认高(px)
		width:                screen.width-793, 				//默认宽(px)
		showTitle:            false, 			//是否显示标题-------标题------------
		titleSlideUpTime:     500, 				//标题上升过程时间(ms)
		titleAnimate:         true, 			//是否启动标题显示时的动画
		tBackgroundColor: 	  '#000',		//标题背景颜色
		tBackgroundOpacity:   '0.6',			//标题背景透明度
		titleColor:   		  '#fff',	   	//标题文字颜色
		titleHeight:   	 	  60,		 		//标题显示高度(px)
		titleAlign:   	 	  'bottom',			//标题对齐方式(top,bottom)
		fontSize:   	  	  16,				//标题文字大小(px)
		fontAlign:   		  'left',			//标题文字对齐方式(left,center,right)
		fontWidth:   	      '420',				//标题文字显示宽度(px)
		isNewLine: 		  	  true,			//标题文字是否可以换行显示
		isIconShow:   		  false,				//是否显示图标--------------图标------------
		iconwidth:   		  37,				//图标显示的宽(px)
		iconheight:   		  7,				//图标显示的高(px)
		iconBorder:   		  '1px solid #fff',//图标显示的边框
		iconColor:   		  '#fff',		//图标显示的颜色
		iconFontSize:   	  0,				//图标文字的大小(px)
		iconFontColor:   	  '#fff',		//图标的文字颜色
		iconBgUrl:   		  'none',			//图标显示的自定义图标(图片url)
		iconOpacity:   	      '0.6',			//图标显示的透明度
		activeIconwidth:   	  37,				//激活时图标宽度(px)
		activeIconheight: 	  7,				//激活时图标高度(px)
		activeIconBorder:     '1px solid #e70012',//激活时图标显示的边框
		activeIconColor:   	  '#50a2ce',		//激活时图标显示的颜色
		activeIconFontColor:  '#50a2ce',		//激活时的文字颜色
		activeIconBgUrl:   	  'none',			//激活时图标显示的自定义图标(图片url)
		activeIconOpacity:    '0.9',			//激活时图标显示的透明度
		iconRight:   	 	  10,   			//图标离右边的距离
		iconBottom:   	  	  25,    			//图标离下边的距离
		iconMargin:   	  	  15,   	 			//图标的间距
	...
	if(settings.showTitle){//显示标题
		...
		//标题样式设置
		$title.css({"position":"absolute","background-color":settings.tBackgroundColor,"height":settings.titleHeight+'px',"width":settings.width+'px',"opacity":settings.tBackgroundOpacity});
		//标题文字样式设置
		...
		if(settings.isNewLine){
			$content.css({"word-break":"break-all","padding-top":"10px","padding-bottom":"5px"});
		}else{
