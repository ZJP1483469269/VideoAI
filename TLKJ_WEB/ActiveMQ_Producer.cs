...
namespace TLKJ.WEB
{
    public class ActiveMQ_Producer
    {
        ...
        private static ActiveMQ_Producer instance = null;
        public static ActiveMQ_Producer getInstance()
        {
            if (instance == null)
            {
                instance = new ActiveMQ_Producer();
            }
            return instance;
        }

        public void SendMessage(String cUSR_ID, String cORG_ID, ActiveMQ_Message vMessage)
        {
            try
            {
                //通过工厂建立连接
